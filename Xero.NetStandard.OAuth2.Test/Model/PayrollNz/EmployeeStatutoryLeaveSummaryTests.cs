...
namespace Xero.NetStandard.OAuth2.Test.Model.PayrollNz
{
    ...
    public class EmployeeStatutoryLeaveSummaryTests : IDisposable
    {
        ...
        public async Task TypeEnum_ValidInput_Deserialises(string input, EmployeeStatutoryLeaveSummary.TypeEnum expected)
        {
            ...
            var actual = await deserializer.Deserialize<EmployeeStatutoryLeaveSummary.TypeEnum>(response);
        ...
        public async Task StatusEnum_ValidInput_Deserialises(string input, EmployeeStatutoryLeaveSummary.StatusEnum expected)
        {
            var jsonContent = $@"""{input}""";
            var response = new HttpResponseMessage(HttpStatusCode.OK)
            {
            ...
            response.EnsureSuccessStatusCode();
            var deserializer = new CustomJsonCodec(new Configuration());
            var actual = await deserializer.Deserialize<EmployeeStatutoryLeaveSummary.StatusEnum>(response);
            Assert.Equal(expected, actual);
        }
