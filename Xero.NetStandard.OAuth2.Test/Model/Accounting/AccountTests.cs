/* 
 * Accounting API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2.0.0
 * Contact: api@xero.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using Xunit;

using System;
using System.Linq;
using System.IO;
using System.Collections.Generic;
using Xero.NetStandard.OAuth2.Api;
using Xero.NetStandard.OAuth2.Model;
using Xero.NetStandard.OAuth2.Client;
using System.Reflection;
using Newtonsoft.Json;
using RestSharp;
using Xero.NetStandard.OAuth2.Model.Accounting;
using Xero.NetStandard.OAuth2.Test.Helpers;

namespace Xero.NetStandard.OAuth2.Test.Model.Accounting
{
    /// <summary>
    ///  Class for testing Account
    /// </summary>
    /// <remarks>
    /// This file is automatically generated by OpenAPI Generator (https://openapi-generator.tech).
    /// Please update the test case below to test the model.
    /// </remarks>
    public class AccountTests : StringTest<Account>, IDisposable
    {

        // public override List<string> GetPropertyNames(){      
        //     return new List<string>(){
        //         "Code",
        //         "Name"
        //     };
        // }


        [Theory]
        [InlineData("a", "a")]
        public void TestString(string value, string expected){
            TestStrings(value, expected);
        }


        // TODO uncomment below to declare an instance variable for Account
        //private Account instance;

        public AccountTests()
        {
            // TODO uncomment below to create an instance of Account
            //instance = new Account();
        }

        public void Dispose()
        {
            // Cleanup when everything is done.
        }

        /// <summary>
        /// Test an instance of Account
        /// </summary>
        [Fact]
        public void AccountInstanceTest()
        {
            // TODO uncomment below to test "IsInstanceOfType" Account
            //Assert.IsInstanceOfType<Account> (instance, "variable 'instance' is a Account");
        }


        /// <summary>
        /// Test the property 'Code'
        /// </summary>
        [Theory]
        [InlineData("091", "091")]
        [InlineData("ABC123", "ABC123")]
        [InlineData(null, "")]
        public void CodeTest(string code, string expected)
        {
            var response = new RestResponse();
            response.Content = $@"{{
                ""Code"": ""{code}""
            }}";

            var deserializer = new CustomJsonCodec(new Configuration());
            var account = deserializer.Deserialize<Account>(response);
            Assert.Equal(expected, account.Code);
        }    

        [Fact]
        public void CodeTestEmpty(){
            var response = new RestResponse();
            response.Content = $@"{{
            }}";

            var deserializer = new CustomJsonCodec(new Configuration());
            var account = deserializer.Deserialize<Account>(response);
            Assert.Equal(null, account.Code);
        }


        /// <summary>
        /// Test the property 'Name'
        /// </summary>
        [Fact]
        public void NameTest()
        {
            // TODO unit test for the property 'Name'
        }
        /// <summary>
        /// Test the property 'AccountID'
        /// </summary>
        [Fact]
        public void AccountIDTest()
        {
            // TODO unit test for the property 'AccountID'
        }
        /// <summary>
        /// Test the property 'Type'
        /// </summary>
        [Fact]
        public void TypeTest()
        {
            // TODO unit test for the property 'Type'
        }
        /// <summary>
        /// Test the property 'BankAccountNumber'
        /// </summary>
        [Fact]
        public void BankAccountNumberTest()
        {
            // TODO unit test for the property 'BankAccountNumber'
        }
        /// <summary>
        /// Test the property 'Status'
        /// </summary>
        [Fact]
        public void StatusTest()
        {
            // TODO unit test for the property 'Status'
        }
        /// <summary>
        /// Test the property 'Description'
        /// </summary>
        [Fact]
        public void DescriptionTest()
        {
            // TODO unit test for the property 'Description'
        }
        /// <summary>
        /// Test the property 'BankAccountType'
        /// </summary>
        [Fact]
        public void BankAccountTypeTest()
        {
            // TODO unit test for the property 'BankAccountType'
        }
        /// <summary>
        /// Test the property 'CurrencyCode'
        /// </summary>
        [Theory]
        [InlineData("AUD", CurrencyCode.AUD)]
        [InlineData("NZD", CurrencyCode.NZD)]
        [InlineData("EMPTY_CURRENCY", CurrencyCode.EMPTY_CURRENCY)]
        public void CurrencyCode_IsValid_DeserializesCorrectly(string code, CurrencyCode expected)
        {   
            var response = new RestResponse();
            response.Content = $@"{{
                ""CurrencyCode"": ""{code}""
            }}";

            var deserializer = new CustomJsonCodec(new Configuration());
            var account = deserializer.Deserialize<Account>(response);
            Assert.Equal(expected, account.CurrencyCode);
        }

   /*     [Fact]
        public void CurrencyCode_IsNull_DeserializesCorrectly()
        {   
            var response = new RestResponse();
            response.Content = $@"{{
                ""CurrencyCode"": ""{null}""
            }}";

            var deserializer = new CustomJsonCodec(new Configuration());
            var account = deserializer.Deserialize<Account>(response);
            Assert.Equal(0, (int)account.CurrencyCode);
        }*/

        [Fact]
        public void CurrencyCode_IsEmpty_DeserializesCorrectly(){
            var response = new RestResponse();
            response.Content = $@"{{
            }}";

            var deserializer = new CustomJsonCodec(new Configuration());
            var account = deserializer.Deserialize<Account>(response);
            Assert.Equal(0, (int)account.CurrencyCode);

        }
                
        /// <summary>
        /// Test the property 'TaxType'
        /// </summary>
        [Fact]
        public void TaxTypeTest()
        {
            // TODO unit test for the property 'TaxType'
        }
        /// <summary>
        /// Test the property 'EnablePaymentsToAccount'
        /// </summary>
        [Fact]
        public void EnablePaymentsToAccountTest()
        {
            // TODO unit test for the property 'EnablePaymentsToAccount'
        }
        /// <summary>
        /// Test the property 'ShowInExpenseClaims'
        /// </summary>
        [Fact]
        public void ShowInExpenseClaimsTest()
        {
            // TODO unit test for the property 'ShowInExpenseClaims'
        }
        /// <summary>
        /// Test the property 'Class'
        /// </summary>
        [Fact]
        public void ClassTest()
        {
            // TODO unit test for the property 'Class'
        }
        /// <summary>
        /// Test the property 'SystemAccount'
        /// </summary>
        [Fact]
        public void SystemAccountTest()
        {
            // TODO unit test for the property 'SystemAccount'
        }
        /// <summary>
        /// Test the property 'ReportingCode'
        /// </summary>
        [Fact]
        public void ReportingCodeTest()
        {
            // TODO unit test for the property 'ReportingCode'
        }
        /// <summary>
        /// Test the property 'ReportingCodeName'
        /// </summary>
        [Fact]
        public void ReportingCodeNameTest()
        {
            // TODO unit test for the property 'ReportingCodeName'
        }
        /// <summary>
        /// Test the property 'HasAttachments'
        /// </summary>
        [Fact]
        public void HasAttachmentsTest()
        {
            // TODO unit test for the property 'HasAttachments'
        }
        /// <summary>
        /// Test the property 'UpdatedDateUTC'
        /// </summary>
        [Fact]
        public void UpdatedDateUTCTest()
        {
            // TODO unit test for the property 'UpdatedDateUTC'
        }
        /// <summary>
        /// Test the property 'AddToWatchlist'
        /// </summary>
        [Fact]
        public void AddToWatchlistTest()
        {
            // TODO unit test for the property 'AddToWatchlist'
        }
        /// <summary>
        /// Test the property 'ValidationErrors'
        /// </summary>
        [Fact]
        public void ValidationErrorsTest()
        {
            // TODO unit test for the property 'ValidationErrors'
        }

    }

}
